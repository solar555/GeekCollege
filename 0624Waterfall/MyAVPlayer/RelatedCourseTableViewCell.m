...
@implementation RelatedCourseTableViewCell
...
#pragma mark 设置model，自动布局
- (void)setRelatedModel:(CDMRelated *)relatedModel{
    _relatedModel = relatedModel;
    [_imgImageView sd_setImageWithURL:[NSURL URLWithString:relatedModel.img]];
    _titleLabel.text = relatedModel.title;
    _timeImageView.image = [UIImage imageNamed:@"ic_time_icon_12x12_"];
    _lessonCountLabel.text = [NSString stringWithFormat:@"%.0lf课时，%.0lf分钟",relatedModel.lessonCount,relatedModel.time];
    _studyNumLabel.text = [NSString stringWithFormat:@"%.0lf人学习",relatedModel.studyNum];
